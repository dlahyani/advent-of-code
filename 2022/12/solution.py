...
def can_move_to_cell(grid: Grid, src: Cell, dst: Cell) -> bool:
    return 0 <= dst[0] < len(grid) and 0 <= dst[1] < len(grid[0]) and \
        ord(grid[dst[0]][dst[1]]) <= ord(grid[src[0]][src[1]])+1


def get_grid_neighbors(grid: Grid, p: Cell) -> tuple[Cell]:
    return tuple(filter(lambda c: can_move_to_cell(grid, p, c),
...
def find_shortest_path(grid: Grid, start: tuple[Cell], end: Cell) -> tuple[int, dict[Cell, Cell]]:
    ...
    while len(queue) > 0:
        ...
        if cell == end: return cell_distance, traceback
