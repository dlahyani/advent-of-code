...
def find_max_height_in_direction(
    tree_map: TreeMap, max_heights_map: MaxHeightMap, row: int, col: int, direction: int
) -> int:
...
def is_tree_visible_from_direction(
    tree_map: TreeMap, max_heights_map: MaxHeightMap, row: int, col: int, direction: int
) -> bool:
...
def is_tree_visible_from_outside(tree_map: TreeMap, max_heights_map: MaxHeightMap, row: int, col: int) -> bool:
    return any(
        [is_tree_visible_from_direction(tree_map, max_heights_map, row, col, d) for d in (UP, RIGHT, DOWN, LEFT)]
    )
