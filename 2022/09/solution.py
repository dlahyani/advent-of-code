...
def simulate_rope_movement(rope_length: int, start_position: Position, instructions: Iterable[str]) -> Iterable[Rope]:
    return reduce(
        lambda rope_movement, i: rope_movement + move_rope(rope_movement[-1], i[0], i[1]),
        map(lambda i: (i[0], int(i[1])), map(lambda i: i.strip().split(), instructions)),
...
def move_rope_one_step(rope: Rope, direction: str) -> Rope:
    return reduce(
        lambda new_rope, i: new_rope + [rope_knot_follow(new_rope[i - 1], rope[i])],
        range(1, len(rope)),
        [move_rope_knot(rope[0], direction)],
    )
